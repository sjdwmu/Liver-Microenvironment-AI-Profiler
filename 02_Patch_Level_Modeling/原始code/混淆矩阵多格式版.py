...
CONFIG = {
    ...
    'MODEL_PATH': r"D:\sjd\project\liver_failure\2.0 data\code\code3.0\36vs81\best_convnext_tiny_10class.pth",
    'OUTPUT_DIR': r"D:\sjd\project\liver_failure\2.0 data\code\code3.0\36vs81"
}
...
def main():
    ...
    if len(all_paths) == 0:
        print("未找到图片，请检查 ROOT_DIRS 路径是否正确。")
        return
    ...
    print(f"\nOverall 10-Class Accuracy: {acc:.4f}\n")
    
    english_classes = [CLASS_TRANSLATION.get(cls, cls) for cls in classes]
    
    print("Classification Report:")
    print(classification_report(all_targets, all_preds, target_names=english_classes, digits=4))
    ...
    print(f"混淆矩阵原始数据已保存至: {csv_save_path}")
    ...
    plt.close()
    print(f"混淆矩阵图已成功保存为 PNG, PDF, SVG 格式至: {CONFIG['OUTPUT_DIR']}")
