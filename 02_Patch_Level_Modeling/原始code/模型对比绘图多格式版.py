...
DATA_DIRS = [
    r"D:\sjd\project\liver_failure\2.0 data\code\code3.0\36vs81\肝衰竭\90生存",
    r"D:\sjd\project\liver_failure\2.0 data\code\code3.0\36vs81\肝衰竭\90死亡"
]
NUM_CLASSES = 10

WEIGHTS = {
    'ResNet50': 'best_resnet50_10class.pth',
    'EfficientNet-V2-S': 'best_efficientnet_v2_s_10class.pth',
    'Swin-T': 'best_swin_t_10class.pth',
    'ConvNeXt-Tiny': 'best_convnext_tiny_10class.pth'
}
...
if __name__ == '__main__':
    ...
    if results_metrics:
        ...
        df_metrics.to_csv('Model_Comparison_Metrics.csv')
        ...
        for i, metric in enumerate(metrics_names):
            ...
            plt.tight_layout()
            plt.savefig(f'Final_{metric.replace(" ", "_")}_Chart.pdf', format='pdf', bbox_inches='tight', transparent=True)
            plt.savefig(f'Final_{metric.replace(" ", "_")}_Chart.svg', format='svg', bbox_inches='tight', transparent=True)
            plt.close()
    ...
    if results_roc:
        ...
        plt.tight_layout()
        plt.savefig('Final_ROC_Comparison.pdf', format='pdf', bbox_inches='tight', transparent=True)
        plt.savefig('Final_ROC_Comparison.svg', format='svg', bbox_inches='tight', transparent=True)
        plt.close()